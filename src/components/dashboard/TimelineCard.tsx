...
export default function DashboardPage() {
    ...
    React.useEffect(() => {
        ...
        if (user.role === 'admin') {
            baseQuery = query(collection(db, 'konsumen'), ...baseConditions);
        } else {
            baseQuery = query(collection(db, 'konsumen'), where('uid', '==', user.id), ...baseConditions);
        }
    ...
    React.useEffect(() => {
        ...
        if (user.role === 'admin') {
             weeklyQuery = query(collection(db, 'konsumen'), ...baseWeeklyConditions);
        } else {
             weeklyQuery = query(collection(db, 'konsumen'), where('uid', '==', user.id), ...baseWeeklyConditions);
        }
